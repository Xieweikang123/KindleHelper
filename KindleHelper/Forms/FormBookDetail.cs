...
namespace KindleHelper
{
    public partial class FormBookDetail : Form
    {
        ...
        public void ShowBook(QueryBookInfo book)
        {
            ...
            if (mBook.title == "《世界通史》" || mBook.title == "世界通史")//《世界通史》
            {
                MessageBox.Show("《世界通史》无法下载！请等待官方推出混合源再试！");
            }
        ...
        private void Backgroundworker_download_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            var result2=Lib2.API.Convert(chaperInfoList);
            book.chapters = result2;
            Global.env.book = book;
            if (ext.ToLower() == ".txt") {
